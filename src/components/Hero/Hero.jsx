...
const Hero = () => {
  ...
  useLayoutEffect(() => {
    const ctx = gsap.context(() => {
      ...
      timeline.current
        .fromTo(".hero-heading", { opacity: 0, y: -50 }, { opacity: 1, y: 0, duration: 1.2 })
        .fromTo(".hero-subheading", { opacity: 0, y: 50 }, { opacity: 1, y: 0, duration: 1 }, "-=0.8")
        .fromTo(".hero-searchbox", { opacity: 0, scale: 0.9 }, { opacity: 1, scale: 1, duration: 1, ease: "elastic.out(1, 0.5)" }, "-=0.6")
        .fromTo(".hero-button", { opacity: 0, scale: 0.8, rotate: -10 }, { opacity: 1, scale: 1, rotate: 0, duration: 1, ease: "back.out(1.7)" }, "-=0.7");
    }, component);

    timeline.current.play(); // Ensure animation runs when loaded
  ...
  return (
    <div
      ...
      style={{ backgroundImage: `url('../../../assets/images/Home Page.webp')` }}
    >
      <div className="text-center text-white w-full max-w-3xl relative">
        <h3 className="hero-subheading font-medium text-xl mb-4 font-AfacadRegular text-black relative inline-block">
          <span className="relative z-10" style={{ WebkitTextStroke: "0.2px rgba(0, 0, 0, 0.2)", letterSpacing: "0.2px" }}>
        ...
        <div className="hero-searchbox relative xs:w-[300px] sm:w-[350px] lg:w-[340px] mx-auto">
          <div className="p-0.5 rounded-3xl bg-gradient-to-b from-[#0E515B] via-[#4EA6B1] to-[#77DAE9]">
            <input type="text" className="w-full h-10 pl-10 p-2 rounded-3xl bg-white focus:outline-none placeholder-gray-400 text-gray-800" placeholder="Search..." />
          </div>
